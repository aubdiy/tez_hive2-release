...
public class UnorderedPartitionedKVWriter extends BaseUnorderedPartitionedKVWriter {
  ...
  private Event generateVMEvent(long[] sizePerPartition) throws IOException {
    ...
    vmBuilder.setNumRecord(
      outputContext.getCounters().findCounter(TaskCounter.OUTPUT_RECORDS).getValue()
      + outputContext.getCounters().findCounter(TaskCounter.OUTPUT_LARGE_RECORDS).getValue());
