...
class FairCartesianProductEdgeManager extends CartesianProductEdgeManagerReal {
  ...
  public EventRouteMetadata routeCompositeDataMovementEventToDestination(int srcTaskId,
    ...
    if (!(lastItemInChunk < firstItemInSrcTask || firstItemInChunk > lastItemInSrcTask)) {
      ...
      int[] targets = new int[count], sources = new int[count];
      for (int i = 0; i < count; i++) {
        targets[i] = firstItem - firstItemInChunk + getItemIdOffset(chunkId) + i;
        sources[i] = firstItem - firstItemInSrcTask + i;
      }
      return EventRouteMetadata.create(count, targets, sources);
    }
    return null;
  }
